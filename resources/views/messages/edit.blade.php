<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('messages.show', $message) }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">
                Volver
            </a>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('messages.update', $message) }}" x-data="messageForm()">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div>
                                ...
                                <select name="type" id="type" 
                                        ...
                                        required>
                                    <option value="email" {{ old('type', $message->type) === 'email' ? 'selected' : '' }}>Email</option>
                                    <option value="whatsapp" {{ old('type', $message->type) === 'whatsapp' ? 'selected' : '' }}>WhatsApp</option>
                                    <option value="sms" {{ old('type', $message->type) === 'sms' ? 'selected' : '' }}>SMS</option>
                                    <option value="notification" {{ old('type', $message->type) === 'notification' ? 'selected' : '' }}>Notificación</option>
                                </select>
                                @error('type')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <select name="priority" id="priority" 
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-gray-900" 
                                        required>
                                    <option value="low" {{ old('priority', $message->priority) === 'low' ? 'selected' : '' }}>Baja</option>
                                    <option value="normal" {{ old('priority', $message->priority) === 'normal' ? 'selected' : '' }}>Normal</option>
                                    <option value="high" {{ old('priority', $message->priority) === 'high' ? 'selected' : '' }}>Alta</option>
                        ...
                        <div class="mb-6">
                            ...
                            @if($message->recipient_type === 'custom')
                                <div class="space-y-2" x-data="{ recipients: {{ json_encode(old('recipients', $message->recipients)) }} }">
                            ...
                            @elseif($message->recipient_type === 'domains')
                                @if($domains->count() > 0)
                                                    ...
                                                    @if(filter_var($email, FILTER_VALIDATE_EMAIL))
                                                        <label class="flex items-center space-x-2 py-1">
                                                            <input type="checkbox" name="recipients[]" value="{{ $email }}" 
                                                                   {{ in_array($email, old('recipients', $message->recipients)) ? 'checked' : '' }}
                                ...
                                @endif
                            @elseif($message->recipient_type === 'quotations')
                                @if($quotations->count() > 0)
                                    <div class="max-h-60 overflow-y-auto border border-gray-300 rounded-md p-3">
                                        @foreach($quotations as $quotation)
                                            <label class="flex items-center space-x-2 py-1">
                                                <input type="checkbox" name="recipients[]" value="{{ $quotation->client_email }}" 
                                                       {{ in_array($quotation->client_email, old('recipients', $message->recipients)) ? 'checked' : '' }}
                                                       class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        <div class="mb-6">
                            <label class="flex items-center">
                                <input type="checkbox" x-model="scheduleMessage" 
                                       {{ old('scheduled_at', $message->scheduled_at) ? 'checked' : '' }}
                                       class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
    ...
    <script>
        function messageForm() {
            return {
                scheduleMessage: {{ old('scheduled_at', $message->scheduled_at) ? 'true' : 'false' }}
            }
        }
    </script>
</x-app-layout>